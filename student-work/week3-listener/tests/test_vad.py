...
from vad import EndpointDetector, TurnState, VadConfig, WebRtcVadClassifier
...
class EndpointDetectorTests(unittest.TestCase):
    ...
    def test_prefix_order_and_onset_frames_are_not_duplicated(self):
        decisions = [False] * 9 + [False, True, True, True, True, False] + [True] * 8 + [False] * 40
        detector = EndpointDetector(classifier=Decisions(decisions))
        results = feed(detector, [frame(i) for i in range(len(decisions))])
    ...
    def test_short_pause_is_preserved_and_does_not_end_turn(self):
        before = [False, True, True, True, True, False] + [True] * 8 + [False] * 39
        detector = EndpointDetector(classifier=Decisions(before + [True] + [False] * 40))
        early = feed(detector, [frame(i) for i in range(len(before))])
        self.assertFalse(any(result.utterance for result in early))
        self.assertEqual(detector.state, TurnState.USER_SPEAKING)
        resumed = frame(200)
        later = feed(detector, [resumed] + [frame(201 + i) for i in range(40)])
        utterance = next(result.utterance for result in later if result.utterance is not None)
        self.assertIn(b"".join(frame(14 + i) for i in range(39)), utterance)
        self.assertTrue(utterance.endswith(resumed))

    def test_forty_silent_frames_end_once_and_are_trimmed(self):
        decisions = [False, True, True, True, True, False] + [True] * 8 + [False] * 41
        detector = EndpointDetector(classifier=Decisions(decisions))
        results = feed(detector, [frame(i) for i in range(len(decisions))])
    ...
    def test_too_short_burst_is_rejected(self):
        decisions = [False, True, True, True, True, False] + [True] * 7 + [False] * 40
        detector = EndpointDetector(classifier=Decisions(decisions))
        results = feed(detector, [frame(i) for i in range(len(decisions))])
