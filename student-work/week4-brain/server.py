"""M4 Brain: hands-free SafeBridge streaming voice cascade."""
...
async def run_turn(websocket:WebSocket,session:ListenerSession,source_pcm:bytes,turn_id:int,
    ...
    try:
        ...
        if not await current_text("turn.done",turn_id=turn_id,timings_ms=timings,segment_count=segment_count,input_frames=input_frames,output_frames=output_frames): return
        if not session.is_current(turn_id,generation): return
