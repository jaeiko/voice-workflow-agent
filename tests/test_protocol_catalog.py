...
class ProtocolCatalogTests(unittest.TestCase):
    ...
    def test_evidence_failure_persists_only_allowlisted_diagnostics(self):
        ...
        self.assertEqual(detail["item_index"], 1)
        self.assertEqual(detail["item_type"], "BeforeStartPrerequisite")
        self.assertEqual(detail["source_page"], 3)
        self.assertNotIn("source_excerpt", detail)
        self.assertNotIn("provider_response", detail)
        self.assertNotIn("source_revision", detail)
        self.assertNotIn("source_hash", detail)
        self.assertNotIn("quote_sha256", detail)
