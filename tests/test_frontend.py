...
"""
    ...
    def test_experiment_timeline_workspace_is_present_and_safely_rendered(self):
        ...
        for required in (
            ...
            "function clearExperimentResumeSelection",
        ...
        self.assertIn('<button id="start" type="button" disabled>', html)
        self.assertNotIn('<div id="state">IDLE</div>', html)
        self.assertNotIn('<strong id="rail-state">IDLE</strong>', html)
        self.assertLess(
...
""" + script + r"""
protocolCatalog.set("candidate-a-curated-development-v1",{protocol_id:"candidate-a-curated-development-v1",title:"Candidate A",revision_id:"fixture-test",available_for_execution:true,development_only:true});
(async()=>{
renderState("IDLE");assert(visibleState==="IDLE"&&ids.state.textContent==="준비"&&!ids.start.disabled&&ids.stop.disabled,"initial controls or researcher-facing state label");
...
await onMessage({data:JSON.stringify({type:"procedure.state",state:{attached:false}})},generation,newSocket);assert(procedureState.attached===false&&ids["procedure-title"].textContent.includes("없음"),"procedure detach not rendered");
    await onMessage({data:JSON.stringify({type:"error",message:"invalid request"})},generation,newSocket);assert(ids["language-status"].textContent.includes("한국어"),"failed request did not preserve fixed korean language UI");assert(visibleState==="MANUAL"&&sessionActive,"accepted-session failure did not preserve the manual fallback state");
