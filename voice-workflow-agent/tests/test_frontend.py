...
assert(JSON.parse(socket.sent.at(-1)).pipeline==="native","native pipeline not requested");
assert(!("language" in JSON.parse(socket.sent.at(-1))),"automatic start sent a manual language");
...
playContext.currentTime=.02;await onMessage({data:JSON.stringify({type:"native.playback.clear",reason:"barge_in",response_id:"native-1",item_id:"item-1"})},generation,newSocket);const truncate=JSON.parse(newSocket.sent.at(-1));assert(nativeStopped&&nativeSources.size===0,"barge-in did not stop playback");assert(truncate.type==="native.playback.truncate"&&truncate.response_id==="native-1"&&Number.isInteger(truncate.audio_end_ms)&&truncate.audio_end_ms===10,"unsafe native truncation metadata");
await onMessage({data:JSON.stringify({type:"native.response.created",turn_id:2,response_id:"native-2"})},generation,newSocket);await onMessage({data:JSON.stringify({type:"native.output.item",turn_id:2,response_id:"native-2",item_id:"item-2"})},generation,newSocket);await onMessage({data:JSON.stringify({type:"native.audio.delta",turn_id:2,response_id:"native-2",item_id:"item-2",sample_rate:24000,byte_length:960})},generation,newSocket);await onMessage({data:new ArrayBuffer(960)},generation,newSocket);const sentBeforeToolClear=newSocket.sent.length;await onMessage({data:JSON.stringify({type:"native.playback.clear",reason:"tool_validation",turn_id:2,response_id:"native-2",item_id:"item-2"})},generation,newSocket);assert(nativeStopped&&newSocket.sent.length===sentBeforeToolClear&&visibleState==="THINKING","tool validation clear was not silent");await onMessage({data:JSON.stringify({type:"native.response.created",turn_id:2,response_id:"native-3"})},generation,newSocket);await onMessage({data:JSON.stringify({type:"native.output.item",turn_id:2,response_id:"native-3",item_id:"item-3"})},generation,newSocket);await onMessage({data:JSON.stringify({type:"native.audio.delta",turn_id:2,response_id:"native-3",item_id:"item-3",sample_rate:24000,byte_length:960})},generation,newSocket);await onMessage({data:new ArrayBuffer(960)},generation,newSocket);await onMessage({data:JSON.stringify({type:"native.response.done",turn_id:2,response_id:"native-3",awaiting_tool_continuation:true})},generation,newSocket);assert(JSON.parse(newSocket.sent.at(-1)).type!=="native.playback.ended","tool continuation started before playback drain");nativeEnded();const playbackEnded=JSON.parse(newSocket.sent.at(-1));assert(playbackEnded.type==="native.playback.ended"&&playbackEnded.response_id==="native-3"&&visibleState==="THINKING","playback completion handshake failed");
