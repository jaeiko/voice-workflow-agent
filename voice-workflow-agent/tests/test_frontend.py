...
await onMessage({data:JSON.stringify({type:"experiment.report.state",configuration_id:1,turn_id:1,generation:10,report:{report_id:"ER-1234567890ABCDEF1234",status:"in_progress",event_count:3,anomaly_count:1,blocker_count:0,reports:[{report_id:"ER-1234567890ABCDEF1234",status:"in_progress"}],events:[{event_type:"anomaly_recorded",step_label:"3",created_at:"2026-07-26T00:00:00Z",user_wording:"색이 변했어요",payload:{timer:{remaining_seconds:12}}}]}})},generation,newSocket);assert(ids["last-report-id"].textContent==="ER-1234567890ABCDEF1234"&&ids["last-report-state"].textContent.includes("이상 1")&&ids["export-report-markdown"].href.endsWith(".md")&&ids["export-report-json"].href.endsWith(".json")&&ids["export-report-csv"].href.endsWith(".csv")&&ids["export-report-docx"].href.endsWith(".docx")&&ids["export-report-json"].attributes["aria-disabled"]==="false"&&ids["experiment-report-list"].children.length===1&&ids["experiment-report-events"].children.length===1,"experiment report state or exports missing");
...
sessionActive=true;playContext={currentTime:0,resume:async()=>{},createBuffer:(_,length,rate)=>({duration:length/rate,getChannelData:()=>new Float32Array(length)}),createBufferSource:()=>({connect(){},start(){},stop(){},disconnect(){},set onended(fn){},get onended(){return null}}),destination:{}};
...
socket=new WS();ids["pipeline-mode"].value="cascade";ids["protocol-id"].value="";await startSession();const unselectedRequest=JSON.parse(socket.sent.at(-1));await onMessage({data:JSON.stringify({type:"session.configuration_required",configuration_id:unselectedRequest.configuration_id,mode:"cascade",language:"ko",protocol_id:null,reason:"protocol_selection_required"})},sessionGeneration,socket);assert(!sessionActive&&acceptedSessionConfiguration===null&&visibleState==="IDLE"&&ids["session-configuration-status"].textContent.includes("정확한 프로토콜"),"mode-only request masqueraded as ready");await stopSession();
