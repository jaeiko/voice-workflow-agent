"""xAI native speech-to-speech relay with fail-closed Voice Workflow Agent boundaries."""
...
class NativeRealtimeConfig:
    ...
    def from_environment(
        cls,vad_settings:NativeVadSettings|None=None
        ...
        try:
            vad=vad_settings or NativeVadSettings.from_environment()
        except ConfigurationError as exc:
            raise NativeRealtimeError(
                f"native voice configuration is invalid: {exc}") from exc
        if not api_key:
        ...
        return cls(
            ...
            vad_prefix_padding_ms=vad.prefix_padding_ms,
            vad_silence_duration_ms=vad.silence_duration_ms,
        )
...
class NativeRealtimeSession:
    ...
    async def playback_ended(self, response_id: str) -> tuple[int, int] | None:
        is_current_playback=response_id==self.playback_response_id
        if is_current_playback:
            self.playback_response_id = None
        if response_id not in self.pending_tool_continuations:
            state=self.responses.get(response_id)
            if (
                not is_current_playback
                or state is None
                or response_id in self.discarded_response_ids
                or state.turn_id!=self.turn_id
                or not state.final_response_done
                or state.first_audio_at is None
                or state.playback_completion_recorded
            ):
                return None
            committed_at=self.turn_committed_at.get(state.turn_id)
            if committed_at is None:
                return None
            state.playback_completion_recorded=True
            return (
                state.turn_id,
                max(0,round((self.clock()-committed_at)*1000)),
            )
    ...
    async def handle_upstream_message(self, raw: str | bytes) -> None:
        ...
        if kind == "input_audio_buffer.speech_started":
            ...
            self.turn_committed_at={
                turn:committed_at
                for turn,committed_at in self.turn_committed_at.items()
        ...
        if kind == "input_audio_buffer.speech_stopped":
            ...
            self.turn_committed_at[self.turn_id]=self.speech_stopped_at
    ...
    async def _forward_audio(self, response_id: str, pcm: bytes) -> None:
        ...
        if state.first_audio_at is None:
            ...
            committed_at=self.turn_committed_at.get(state.turn_id)
            first_audio_ms = (
                round((state.first_audio_at-committed_at)*1000)
    ...
    async def _handle_response_done(self, message: dict[str, Any]) -> None:
        ...
        committed_at=self.turn_committed_at.get(state.turn_id)
        total_ms=(
            round((self.clock()-committed_at)*1000)
            if committed_at is not None else None)
        first_audio_ms = (
            round((
                state.first_audio_at-self.turn_committed_at[state.turn_id]
            )*1000)
            if (
                state.first_audio_at is not None
                and state.turn_id in self.turn_committed_at
            )
            else None
        )
        await self.sender.text(
        ...
        state.final_response_done=True
        if response_id == self.active_response_id:
            self.active_response_id = None
            self.active_item_id = None
