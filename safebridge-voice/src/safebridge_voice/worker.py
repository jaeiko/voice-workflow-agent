"""SafeBridge Voice handoff worker.
...
from safebridge_voice.tools import INBOX_PATH, OUTBOX_DIR, PROCESSED_PATH, STATUS_DIR

PROJECT_ROOT = Path(__file__).resolve().parents[2]
load_dotenv(PROJECT_ROOT / ".env")

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("safebridge.worker")

POLL_SECONDS = 2
MAX_ATTEMPTS = 3
URGENCY_ORDER = {"emergency": 0, "urgent": 1, "routine": 2}

HANDOFF_PROMPT = """You are the SafeBridge Voice handoff worker. Convert one queued lab safety report JSON into a concise Korean handoff note for a lab manager. Include the report id, reported location, event summary, named material or equipment when present, exposure status, urgency, and the fact that details still require human verification. Distinguish reported facts from unknown information. Do not invent causes, diagnoses, procedures, phone numbers, laws, exposure limits, or completed actions. Do not say the area is safe or approve work resumption. For emergency severity, put an immediate human contact request first. Plain text only, no Markdown, no subject line, no placeholders. End with 'SafeBridge Voice 자동 인계'."""
...
def write_handoff_email(
    ...
    message["Subject"] = (
        f"{prefix}SafeBridge Voice 보고 {report['id']} — {report['location']}"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SafeBridge Voice report worker")
