"""Optional in-memory Moss reranking for approved SafeBridge catalog sections.

SQLite remains the policy authority. Moss only ranks the sections that already
passed SafeBridge's product, facility, language, approval, version, scope, and
topic gates.
"""
...
log = logging.getLogger("safebridge.moss")
...
class MossSettings:
    ...
    def from_environment(cls) -> "MossSettings":
        enabled = _boolean_env("SAFEBRIDGE_MOSS_ENABLED")
        if not enabled:
        ...
        raw_scopes = os.environ.get(
            "SAFEBRIDGE_MOSS_ALLOWED_SCOPES",
        ...
        if not allowed_scopes or not allowed_scopes.issubset(MOSS_CAPABLE_SCOPES):
            raise ValueError("SAFEBRIDGE_MOSS_ALLOWED_SCOPES is invalid")
        ...
        return cls(
            ...
            alpha=_bounded_float("SAFEBRIDGE_MOSS_ALPHA", 0.65, 0.0, 1.0),
            candidate_limit=_bounded_int(
                "SAFEBRIDGE_MOSS_CANDIDATE_LIMIT", 64, 3, 100
            ),
            query_timeout_seconds=_bounded_float(
                "SAFEBRIDGE_MOSS_QUERY_TIMEOUT_MS", 250.0, 10.0, 5000.0
            )
            / 1000,
            load_timeout_seconds=_bounded_float(
                "SAFEBRIDGE_MOSS_LOAD_TIMEOUT_SECONDS", 60.0, 1.0, 300.0
            ),
            auto_refresh=_boolean_env("SAFEBRIDGE_MOSS_AUTO_REFRESH"),
            refresh_seconds=_bounded_int(
                "SAFEBRIDGE_MOSS_REFRESH_SECONDS", 600, 60, 86400
            ),
        )
...
class MossRuntime:
    ...
    def _sdk_types(self) -> tuple[Callable[[str, str], Any], Callable[..., Any]]:
        ...
        try:
            ...
            raise RuntimeError(
                "Moss SDK is not installed; install safebridge-voice[moss]"
            ) from exc
