...
SafeBridge Voice can optionally use [Moss](https://github.com/usemoss/moss) to
...
raises an error, SafeBridge returns the existing deterministic SQLite ordering.
...
```bash
cd ~/voice-ai-course/safebridge-voice
...
export SAFEBRIDGE_SAFETY_CATALOG="$demo_dir/moss_demo_catalog.sqlite"
export SAFEBRIDGE_FACILITY_ID="MOSS-DEMO-FACILITY"
export SAFEBRIDGE_USAGE_SCOPE="demo"
export SAFEBRIDGE_SESSION_LANGUAGE="ko"
export SAFEBRIDGE_ALLOWED_LANGUAGES="ko"
unset SAFEBRIDGE_PROCEDURE_CATALOG SAFEBRIDGE_PROCEDURE_STORE
```
...
index containing documents that are not SafeBridge-managed records for the exact
requested scope.
...
```dotenv
SAFEBRIDGE_MOSS_ENABLED=true
SAFEBRIDGE_MOSS_ALLOWED_SCOPES=demo,reference_only
SAFEBRIDGE_MOSS_ALPHA=0.65
SAFEBRIDGE_MOSS_CANDIDATE_LIMIT=64
SAFEBRIDGE_MOSS_QUERY_TIMEOUT_MS=250
SAFEBRIDGE_MOSS_LOAD_TIMEOUT_SECONDS=60
SAFEBRIDGE_MOSS_AUTO_REFRESH=false
SAFEBRIDGE_MOSS_REFRESH_SECONDS=600
```
...
```dotenv
SAFEBRIDGE_MOSS_ALLOWED_SCOPES=operational
```
...
```text
Moss index loaded in memory: safebridge-approved-safety
```
...
- `SAFEBRIDGE_MOSS_ALPHA=0.65` keeps both semantic and keyword signal.
