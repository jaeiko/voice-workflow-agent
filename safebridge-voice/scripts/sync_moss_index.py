#!/usr/bin/env python3
"""Sync approved SafeBridge catalog sections to an explicitly chosen Moss index."""
...
from safebridge_voice.moss_retrieval import (  # noqa: E402
    MOSS_CAPABLE_SCOPES,
    ...
    if existing is None:
        ...
        unmanaged = [
            ...
            if (
                ...
                or item.metadata.get("safebridge_key") != item.id
                or item.metadata.get("usage_scope") != args.usage_scope
            )
        ]
        if unmanaged:
            raise RuntimeError(
                "existing index is not exclusively managed by SafeBridge for "
